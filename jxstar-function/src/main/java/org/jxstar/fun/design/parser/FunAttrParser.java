...
public class FunAttrParser {
	...
	public static String parseAttr(String funId, String attrType) {
		...
		for (Map<String,String> mpAttr : lsAttr) {
			...
			String attrTpl = queryAttrTpl(attrName, attrType);
	...
	private static List<Map<String,String>> queryAttr(String funId, String attrType) {
		String sql = "select attr_name, attr_value from fun_attr where fun_id = ? and attr_type = ? order by attr_no";
	 ...
	 * @return
	 */
	private static String queryAttrTpl(String attrName, String attrType) {
		String sql = "select attr_tpl from fun_attr where fun_id = 'basestatic' and attr_name = ? and attr_type = ?";
		DaoParam param = _dao.createParam(sql);
