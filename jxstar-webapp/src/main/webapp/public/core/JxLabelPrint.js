...
(function(){
	...
	var getPrintCtl = function() {
		...
		if (Ext.isEmpty(el)) {
			var html = '<object id="printcodectl" classid="clsid:7EDC7219-8CEE-451f-9C21-98C812DD8CAE" width="0" height="0"></object>';
	...
	Ext.apply(JxLabelPrint, {
		 ...
		 * isView -- 1表示预览，否则为打印
		 **/
		print: function(grid, headFile, dataFile, tableName, filedNames, isView) {
			if (Ext.isIE == false) {
				JxHint.alert('条码标签打印控件不支持非IE浏览器！');
				return;
			}
